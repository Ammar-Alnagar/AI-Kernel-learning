 ...
 *   This maps to: NVIDIA DL Software Engineer — "FlashAttention-2 GEMM with CuTe"
 *
 * MENTAL MODEL:
...
__global__ void tiled_gemm_kernel(half* gmem_A, half* gmem_B, float* gmem_C) {
    ...
    using MMA_Atom = MMA_Atom<
        MMA_Traits<Shape<Int<16>, Int<8>, Int<16>>,  // m16n8k16
                   Element_t<float>, Element_t<float>, Element_t<float>>  // Using FP32 for simplicity
    >;
...
int main() {
    ...
    printf("Note: Small GEMM - overhead dominated. See Projects for real TFLOPS.\n");
 ...
 *     Answer: tiled_mma (the operator), A_frag, B_frag, C_frag (register tensors)
 * 
