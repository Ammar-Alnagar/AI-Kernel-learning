 ...
 *   FlashAttention-2 and TRT-LLM kernels start by defining the layout of Q, K, V tensors.
 *   Row-major [seqlen, head_dim] is the standard layout for attention inputs.
 *   This maps to: NVIDIA DL Software Engineer (Inference) — "CuTe layout algebra for FlashAttention"
 *
 * MENTAL MODEL:
 ...
 *   Row-major [M, N]: stride = (N, 1) — move by N to go down a row, by 1 to go right
 */
...
__global__ void basic_layouts_kernel() {
    // MENTAL MODEL: make_layout(shape, stride) creates a mapping from indices to offsets
    // For row-major, stride is (N, 1) for shape (M, N)
    ...
    // MENTAL MODEL: shape(layout) returns the shape tuple, stride(layout) returns stride tuple
...
int main() {
    ...
    // Look for: CUDA API calls, kernel launch overhead (minimal for this simple kernel)
