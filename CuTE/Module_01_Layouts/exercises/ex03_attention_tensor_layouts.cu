/*
 * WHAT THIS TEACHES:
 *   - Construct 4D layouts for attention tensors [batch, heads, seqlen, head_dim]
 ...
 *   Understanding the memory layout is critical for correct tiling and coalesced access.
...
__global__ void attention_layouts_kernel() {
    ...
    auto layout_4d = make_layout(make_shape(
        Int<2>{},   // batch
        Int<4>{},   // heads
        Int<8>{},   // seqlen
        Int<16>{}   // head_dim
    ...
    printf("=== 4D Attention Tensor Layout [batch=2, heads=4, seqlen=8, head_dim=16] ===\n");
    print(layout_4d);
    printf("\n");
    ...
    printf("Offset for (1, 2, 4, 8): %d (expected %d) - %s\n", 
           offset, expected, (offset == expected) ? "MATCH" : "MISMATCH");
    
    // MENTAL MODEL: FlashAttention-2 typically fuses batch*heads into one dimension
    // This simplifies tiling: [batch*heads, seqlen, head_dim]
    auto layout_fused = make_layout(make_shape(
        Int<8>{},   // batch * heads = 2 * 4 = 8
        Int<8>{},   // seqlen
        Int<16>{}   // head_dim
    ));
    ...
    int tiled_off = layout_tiled(
        make_coord(Int<0>{}, Int<1>{}, Int<0>{}),  // batch*heads=0, tile=1, head_dim=0
