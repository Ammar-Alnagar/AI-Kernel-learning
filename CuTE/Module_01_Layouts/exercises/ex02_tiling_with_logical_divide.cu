 ...
 *   logical_divide(shape, tile_shape) expresses this tiling without manual index math.
 *   This maps to: NVIDIA DL Software Engineer — "FlashAttention tiling with CuTe"
 *
 * MENTAL MODEL:
...
__global__ void tiling_kernel() {
    ...
    // Result: 4 tiles in dim 0 (128/32=4), 4 tiles in dim 1 (64/16=4) = 16 tiles total
    auto layout_tiled = logical_divide(layout_big, make_shape(Int<32>{}, Int<16>{}));
...
int main() {
    ...
    // Q3: Does layout_tiled(tile_coord, elem_coord) equal layout_big(global_coord)?
