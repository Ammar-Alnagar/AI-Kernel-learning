 ...
 *   - Express GQA (Grouped Query Attention) where multiple query heads share K/V
 *   - Understand how stride-0 enables efficient memory access without copies
 *
 * WHY THIS MATTERS FOR LLM INFERENCE:
 *   GQA is used in Llama-2-70B, Mistral, and other modern LLMs to reduce KV cache size.
 *   8 query heads might share 2 K/V heads, reducing memory by 4x.
 ...
 *   This maps to: Cerebras LLM Inference Performance — "FlashAttention variants including GQA"
 *
 * MENTAL MODEL:
 *   GQA: 8 query heads, 2 K/V heads -> each K/V head is shared by 4 query heads
 *   Query layout: [8, seqlen, head_dim] — normal row-major
 *   K/V layout: [2, seqlen, head_dim] with head stride = 0 for the group dimension
 *   When query head q accesses K/V, it uses K/V head index q / 4 (integer division)
 */
...
__global__ void gqa_stride_zero_kernel() {
    ...
    auto layout_kv_normal = make_layout(make_shape(
        Int<4>{},   // heads
        Int<8>{},   // seqlen
        Int<16>{}   // head_dim
    ));
    ...
    auto layout_kv_gqa = make_layout(make_shape(
        Int<2>{},   // K/V heads (fewer than query heads)
        Int<8>{},   // seqlen
        Int<16>{}   // head_dim
    ));
    ...
    auto layout_gqa_broadcast = make_layout(
        make_shape(Int<2>{}, Int<4>{}, Int<8>{}, Int<16>{}),  // [groups, intra, seq, dim]
        make_stride(Int<128>{}, Int<0>{}, Int<16>{}, Int<1>{}) // intra has stride 0!
    );
    
    printf("\n=== GQA Broadcast Layout [groups=2, intra=4, seqlen=8, head_dim=16] ===\n");
