...
int main() {
  ...
  // TASK 2: Create a padded 8x8 layout with stride 9 (adding 1 element padding
  // per row) This helps avoid bank conflicts in shared memory Hint: Use
  // make_stride(Int<9>{}, Int<1>{}) for row-major with padding
  // TODO: Uncomment and complete:
  // auto layout_padded = make_layout(make_shape(Int<8>{}, Int<8>{}),
  // make_stride(Int<9>{}, Int<1>{}));
  auto layout_padded = make_layout(make_shape(8, 8), make_stride(9, 1));
  ...
  std::cout << "Standard layout: " << standard_elements << " elements"
            << std::endl;
  std::cout << "Padded layout: " << padded_elements << " elements" << std::endl;
  std::cout << "Overhead: " << overhead << " elements (" << overhead_percent
            << "%)" << std::endl;
  std::cout << std::endl;
  ...
  // auto layout_strided = make_layout(make_shape(Int<8>{}, Int<16>{}),
  //                                   make_stride(Int<32>{}, Int<1>{}));

  auto layout_strided = make_layout(make_shape(16, 16), make_stride(32, 1));
  std::cout << "Task 4 - Strided Layout (every other row):" << std::endl;
  std::cout << "Layout: " << layout_strided << std::endl;
  std::cout << "This layout accesses rows 0, 2, 4, 6, 8, 10, 12, 14 of a "
               "16-column matrix"
            << std::endl;
  std::cout << std::endl;
  ...
  std::cout << "When 32 threads access a column in an 8x8 standard layout:"
            << std::endl;
  std::cout << "  Thread 0 accesses offset " << layout_standard(0, 0)
            << " -> bank " << (layout_standard(0, 0) % 32) << std::endl;
  std::cout << "  Thread 1 accesses offset " << layout_standard(1, 0)
            << " -> bank " << (layout_standard(1, 0) % 32) << std::endl;
  std::cout << "  Thread 2 accesses offset " << layout_standard(2, 0)
            << " -> bank " << (layout_standard(2, 0) % 32) << std::endl;
  std::cout << "  (No conflict - each thread accesses different bank)"
            << std::endl;
  std::cout << std::endl;

  std::cout << "When 32 threads access a row in an 8x8 standard layout:"
            << std::endl;
  ...
  std::cout << "2. Padding trades memory for better access patterns"
            << std::endl;
  std::cout << "3. Padding helps avoid bank conflicts in shared memory"
            << std::endl;
