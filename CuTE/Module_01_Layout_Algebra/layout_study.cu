...
// Function to demonstrate how a layout maps logical coordinates to memory
// offsets
void demonstrate_layout_mapping() {
  ...
  auto layout = make_layout(make_shape(cute::Int<4>{}, cute::Int<3>{}),
                            cute::GenRowMajor{});
...
void demonstrate_column_major() {
  ...
  auto col_major_layout =
      make_layout(make_shape(Int<4>{}, Int<3>{}), GenColMajor{});
...
void demonstrate_custom_strides() {
  ...
  // Layout with padding: 4x3 matrix but with stride 5 for rows (padding of 2
  // elements) This simulates a padded matrix where each row has extra space
  auto padded_layout = make_layout(make_shape(Int<4>{}, Int<3>{}),
  ...
  std::cout << "\nMapping logical coordinates to memory offsets (with padding):"
            << std::endl;
  for (int i = 0; i < 4; ++i) {
    for (int j = 0; j < 3; ++j) {
...
void demonstrate_hierarchical_layout() {
  ...
  // Explanation of tiling concepts
  std::cout << "Simple tiling concept: dividing a 4x6 layout into 2x3 blocks\n";
  std::cout << "This would result in 2 blocks along first dimension and 2 "
               "along second\n";
  std::cout << "(2x2) outer tiles containing (2x3) elements each\n";
}
...
int main() {
  ...
  std::cout << "\n=== Summary ===" << std::endl;
  std::cout << "1. Layouts map logical coordinates (i,j) to memory offsets"
            << std::endl;
  std::cout << "2. Shape defines dimensions, Stride defines step sizes"
            << std::endl;
  std::cout << "3. cute::print() is essential for debugging layout mappings"
            << std::endl;
  std::cout << "4. Hierarchical layouts enable complex tiling strategies"
            << std::endl;
