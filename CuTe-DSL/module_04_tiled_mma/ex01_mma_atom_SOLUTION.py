...
# Your answer: 16x16x16 (M=16, N=16, K=16)

# Q2: Why is the accumulator typically FP32 when inputs are FP16?
# Your answer: FP32 accumulation provides numerical stability for large
#              reductions. FP16 would overflow/underflow for large sums.

# Q3: What is the difference between Mma_Sm80 and Mma_Sm90?
# Your answer: SM90 (Hopper) has new tensor core instructions with improved
#              throughput and async capabilities. Different instruction encoding.
...
def kernel_mma_atom(
    ...
    mma_atom = cute.MMA_atom(MMA_OP, A_DTYPE, B_DTYPE, C_DTYPE)
    
    # --- Step 2: Initialize fragments ---
    # A and B are already initialized to 1.0 from host
    # C is already initialized to 0.0 from host
    
    # --- Step 3: Execute MMA: C = A @ B + C ---
    cute.gemm(mma_atom, c_frag, a_frag, b_frag, c_frag)
    
    # --- Step 4: Verify result ---
    results[0] = c_frag[0, 0]
    results[1] = 16.0  # Expected: sum of 16 products of 1*1
    results[2] = c_frag[7, 7]  # Another element
    results[3] = 16.0  # All elements should be 16.0
...
def run():
    ...
    print(f"    C[7,7]: {results_cpu[2]:.1f} (expected: {results_cpu[3]:.1f})")
    ...
    passed = (
        ...
        abs(results_cpu[2] - expected) < 0.1 and
