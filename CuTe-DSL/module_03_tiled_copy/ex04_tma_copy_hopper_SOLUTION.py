...
# Your answer: TMA is hardware-accelerated with dedicated tensor cores.
#              It handles address calculation and provides native barrier
#              integration for async synchronization.

# Q2: How does TMA integrate with barrier synchronization?
# Your answer: TMA copies signal the barrier on completion. Consumer
#              warps call barrier.sync() to wait for the data.

# Q3: In FlashAttention-3's warp specialization, which warp role uses TMA?
# Your answer: DMA (Data Movement) warps use TMA to load QKV from GMEM→SMEM.
#              MMA warps wait on barriers and do the attention compute.
...
def kernel_tma_copy(
    ...
    if tid == 0:
        ...
        results[2] = dst_tensor[M - 1, N - 1]
        results[3] = float(TOTAL - 1)  # Expected
