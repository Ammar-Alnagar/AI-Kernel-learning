...
# Your answer: UniversalCopy is a generic copy that works for any dtype.
#              VectorizedCopy uses wider load/store instructions (e.g., 128-bit)
#              for better throughput when alignment permits.
...
# Your answer: 128-bit / 16-bit = 8 elements per vectorized load

# Q3: Why do copy atoms specify both source and destination dtypes?
# Your answer: To support type conversion during copy (e.g., FP16→FP32).
#              Also ensures correct load/store instruction selection.
...
def kernel_copy_atom(
    ...
    copy_atom = cute.Copy_atom(COPY_OP, SRC_DTYPE, DST_DTYPE)
    ...
    for i in range(4):
        cute.copy(copy_atom, src_tensor[i], dst_tensor[i])
    
    # --- Step 4: Verify copy by reading back ---
    results[0] = dst_tensor[0]
    results[1] = dst_tensor[1]
    results[2] = dst_tensor[2]
    results[3] = dst_tensor[3]
