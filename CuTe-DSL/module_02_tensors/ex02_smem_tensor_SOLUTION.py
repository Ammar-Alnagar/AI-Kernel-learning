...
# Your answer: 192 KB (configurable as 164KB L1 + 128KB SMEM or other splits)

# Q2: Why do we need to explicitly allocate SMEM vs GMEM which is allocated by PyTorch?
# Your answer: SMEM is on-chip, per-SM memory with explicit allocation.
#              It's managed differently from GMEM which is device-wide.
...
# Your answer: Reading along columns causes bank conflicts (stride-32 access).
#              Reading along rows is coalesced (stride-1 access).
...
def kernel_smem_tensor(
    ...
    if cute.thread_idx() == 0:
        for i in range(M):
            for j in range(N):
                smem_tensor((i, j)) = float(i + j)
    
    # Ensure all threads see the initialized data
    cute.sync_threads()
    
    # --- Step 4: Read elements from SMEM ---
    results[0] = smem_tensor((10, 5))
    results[1] = smem_tensor((20, 10))
    results[2] = smem_tensor((30, 15))
