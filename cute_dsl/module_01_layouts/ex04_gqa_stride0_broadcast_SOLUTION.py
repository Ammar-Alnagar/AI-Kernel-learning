...
# Your answer: Both are 10. stride-0 on heads means head index contributes 0.

# Q2: Why do both indices from Q1 map to the same linear index?
#     What does this mean for memory loads?
# Your answer: Because stride-0 means the head dimension doesn't affect the
#              linear index. This means we load from the same physical location
#              regardless of head index — achieving broadcast without copying.
...
# Your answer: Use stride-0 on a layout with shape (8, 64). The kernel logic
#              handles mapping query heads to KV heads via modulo. The layout
#              provides the broadcast semantics for the actual loads.

# Q4: What is the memory traffic reduction factor when using stride-0
#     broadcast for GQA with 8 query heads and 2 KV heads?
# Your answer: 8/2 = 4× reduction. We load 2 KV heads instead of 8.
