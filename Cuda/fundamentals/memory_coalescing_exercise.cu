...
__global__ void coalescedCopy(float* input, float* output, int n) {
    ...
    if (tid < n) {
        ...
        // output[tid] = input[tid * stride];  // This is just an example of bad access
...
int main() {
    ...
    cudaMemcpy(h_output_uncoalesced, d_output_uncoalesced, size, cudaMemcpyDeviceToHost);
    ...
    // Cleanup
    free(h_input); free(h_output_coalesced); free(h_output_uncoalesced);
    cudaFree(d_input); cudaFree(d_output_coalesced); cudaFree(d_output_uncoalesced);
