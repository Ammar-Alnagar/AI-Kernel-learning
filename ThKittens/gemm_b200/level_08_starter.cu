/*
 * ThunderKittens Fill-in Tutorial (B200) - Level 08
 * Goal: Add epilogue pipelining
 *
 * How to use:
  ...
  if (warpid == 0 && laneid == 0) {
    ...
    for (int iter_k = 0; iter_k < num_k_iters; iter_k++) {
      ...
      if (stage == PIPE_STAGES - 1)
        ...
        if (iter_k == 0) {
        // TODO(L08-1): bootstrap MM op at iter 0.
        // mm_AB(accum, a_smem[stage], b_smem[stage], inputs_finished[stage]);
      } else {
        // TODO(L08-2): steady-state MMA op at iter > 0.
...
#pragma unroll
  for (int i = 0; i < EPI_PIPE_DEPTH; i++) {
    ...
    if (wg_laneid == 0) {
      // TODO(L08-3): pipelined epilogue store for fragment i.
