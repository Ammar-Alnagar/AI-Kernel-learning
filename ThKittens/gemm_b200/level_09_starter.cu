/*
 * ThunderKittens Fill-in Tutorial (B200) - Level 09
 * Goal: Use cluster-level cooperation + warpgroup parallelism
 *
 * How to use:
 * 1) Open this starter file and replace TODO markers with working code.
 * 2) If blocked, compare against level_09_solution.cu in the same folder.
 * 3) Build with: make LEVEL=09 TRACK=starter clean && make LEVEL=09 TRACK=starter run
 *
 * Exercise rules:
    ...
    if (warpgroup::groupid() == NUM_CONSUMERS) {
        ...
        if (warp::laneid() == 0 && warpgroup::warpid() == 3) {
            ...
            for (int idx = 0; idx < iters_per_task; idx++) {
                ...
                #pragma unroll
                for (int i = 0; i < NUM_CONSUMERS; i++) {
                    // TODO(L09-1): cluster load A panel for consumer i.
                    // tma::cluster::load_async(
                    //     a_smem[input_ring][i], A_layout,
                    //     {(tile_coord.x * 2 + cta_rank) * NUM_CONSUMERS + i, idx},
                    //     inputs_arrived[input_ring], (uint16_t)(1 << cta_rank), 0
                    // );
                }

                // TODO(L09-2): cluster load shared B panel.
                // tma::cluster::load_async(
                //     b_smem[input_ring], B_layout,
