/*
 * ThunderKittens Fill-in Tutorial (H100) - Level 08
 * Goal: Add multi-consumer warpgroups and wide-tile stores
 *
 * How to use:
 * 1) Open this starter file and replace TODO markers with working code.
 * 2) If blocked, compare against level_08_solution.cu in the same folder.
 * 3) Build with: make LEVEL=08 TRACK=starter clean && make LEVEL=08 TRACK=starter run
 *
 * Exercise rules:
...
__global__ void kernel(const __grid_constant__ matmul_globals g) {
    ...
    if (is_consumer) {
        // TODO(L08-1): zero consumer accumulator state.
    ...
    for (int tile = 0; tile < num_tiles; ++tile, tic^=1, toc^=1) {
        ...
        if (is_producer) {
            ...
            // TODO(L08-2): consumer MMA over one A tile and the wide B tile pack.
            // warpgroup::mma_AB(
            //     C_accum,
            //     As[tic][consumer_idx],
    ...
    if (is_consumer) {
        ...
        if (warpid % 4 == 0) {
            for (int n = 0; n < N_BLOCK; n++) {
                // TODO(L08-3): async store each output subtile for this consumer row.
