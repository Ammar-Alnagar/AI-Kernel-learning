/*
 * ThunderKittens Fill-in Tutorial (H100) - Level 01
 * Goal: Simple for-loop GEMM (float) baseline
 *
 * How to use:
 * 1) Open this starter file and replace TODO markers with working code.
 * 2) If blocked, compare against level_01_solution.cu in the same folder.
 * 3) Build with: make LEVEL=01 TRACK=starter clean && make LEVEL=01 TRACK=starter run
 *
 * Exercise rules:
