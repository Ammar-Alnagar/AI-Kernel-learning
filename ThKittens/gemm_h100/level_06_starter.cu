/*
 * ThunderKittens Fill-in Tutorial (H100) - Level 06
 * Goal: Add TMA + double buffering
 *
 * How to use:
...
__global__ void kernel(const __grid_constant__ matmul_globals g) {
    ...
    if (threadIdx.x == 0) { // this should be on thread and not warp (SA: note)
        ...
        // TODO(L06-1): preload first A tile into ping buffer.
        // tma::load_async(As[tic], g.A, {0, 0, row, 0}, bar);
        // TODO(L06-2): preload first B tile into ping buffer.
    ...
    for (int tile = 0; tile < num_tiles; ++tile, tic^=1, toc^=1) {
        ...
        // TODO(L06-3): compute on the active pipeline stage.
