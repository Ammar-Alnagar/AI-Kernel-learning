/*
 * ThunderKittens Fill-in Tutorial (H100) - Level 07
 * Goal: Add producer/consumer warp specialization
 *
 * How to use:
...
__global__ void kernel(const __grid_constant__ matmul_globals g) {
    ...
    if (warpgroupid == 0) { // producer
        // TODO(L07-1): reduce producer registers to free consumer capacity.
        ...
        // TODO(L07-2): increase consumer registers for MMA throughput.
        ...
        for (int tile = 0; tile < num_tiles; ++tile, ++qidx) {
            if (qidx == QSIZE) { qidx = 0; p ^= 1; }
            wait(full[qidx], p);
            // TODO(L07-3): consumer issues warpgroup MMA from queue slot qidx.
